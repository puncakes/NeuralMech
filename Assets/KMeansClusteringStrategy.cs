...
public class KMeansClusteringStrategy : ISpeciationStrategy
{
    ...
    #region ISpeciationStrategy<NEATGenome> Members
    ...
    /// </summary>
    public IList<Specie> InitializeSpeciation(IList<NEATGenome> genomeList, int specieCount)
    {
        ...
        List<Specie> specieList = new List<Specie>(specieCount);
        for (int i = 0; i < specieCount; i++)
        {
            specieList.Add(new Specie((uint)i, i, initSpeciesCapacity));
        }
    ...
    /// </summary>
    public void SpeciateGenomes(IList<NEATGenome> genomeList, IList<Specie> specieList)
    {
        //Debug.Assert(SpeciationUtils.TestEmptySpecies(specieList), "SpeciateGenomes(IList<NEATGenome>,IList<Species<NEATGenome>>) called with non-empty species");
        //Debug.Assert(genomeList.Count >= specieList.Count, string.Format("SpeciateGenomes(IList<NEATGenome>,IList<Species<NEATGenome>>). Species count [{0}] is greater than genome count [{1}].", specieList.Count, genomeList.Count));
    ...
    /// offspring.
    /// </summary>
    public void SpeciateOffspring(IList<NEATGenome> offspringList, IList<Specie> specieList)
    {
        ...
        //Debug.Assert(SpeciationUtils.TestPopulatedSpecies(specieList), "SpeciateOffspring(IList<NEATGenome>,IList<Species<NEATGenome>>) called with an empty specie.");
        ...
        foreach (NEATGenome genome in offspringList)
        {
            Specie closestSpecie = FindClosestSpecie(genome, specieList);
            closestSpecie.GenomeList.Add(genome);
            genome.SpecieIdx = closestSpecie.Idx;
        ...
        List<NEATGenome> genomeList = new List<NEATGenome>(genomeCount);
        foreach (Specie specie in specieList)
        {
    ...
    /// </summary>
    private void SpeciateUntilConvergence(IList<NEATGenome> genomeList, IList<Specie> specieList)
    {
        List<Specie> emptySpecieList = new List<Specie>();
        int specieCount = specieList.Count;
        ...
        for (int loops = 0; loops < __MAX_KMEANS_LOOPS; loops++)
        {
            ...
            for (int i = 0; i < specieCount; i++)
            {
                ...
                Specie specie = specieList[i];
                specie.GenomeList.RemoveAll(delegate (NEATGenome genome)
                {
            ...
            if (0 != emptySpecieList.Count)
            {
                ...
                NEATGenome[] genomeByDistanceArr = GetGenomesByDistanceFromSpecie(genomeList, specieList);
                ...
                for (int i = 0; i < emptySpecieCount; i++)
                {
                    ...
                    Specie emptySpecie = emptySpecieList[i];
    ...
    /// </summary>
    private CoordinateVector CalculateSpecieCentroid(Specie specie)
    {
        ...
        List<NEATGenome> genomeList = specie.GenomeList;
    ...
    /// </summary>
    private NEATGenome[] GetGenomesByDistanceFromSpecie(IList<NEATGenome> genomeList, IList<Specie> specieList)
    {
        // Build a list of all genomes paired with their distance from their centriod.
        int genomeCount = genomeList.Count;
        GenomeDistancePair[] genomeDistanceArr = new GenomeDistancePair[genomeCount];
        ...
        NEATGenome[] genomeArr = new NEATGenome[genomeCount];
        for (int i = 0; i < genomeCount; i++)
        {
    ...
    /// </summary>
    private Specie FindClosestSpecie(NEATGenome genome, IList<Specie> specieList)
    {
        // Measure distance to first specie's centroid.
        Specie closestSpecie = specieList[0];
